...
void comare_hier_dense_crf() {
	...
	cv::Mat raw_rgb_image = cv::imread(rgb_image_path, cv::IMREAD_COLOR );
	cv::Mat superpixel_rgb = cv::imread(superpixel_rgb_path, cv::IMREAD_COLOR );	
...
// 	crf.addBilateral_pos(appear_x_stddev ,appear_y_stddev);
	...
	cv::Mat unary_im;
	unary_im = cv::imread(unary_im_path, cv::IMREAD_COLOR );
